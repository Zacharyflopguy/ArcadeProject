...
public class ShadowAI : MonoBehaviour
{
    ...
    public int cloneCount = 3; // Number of shadow clones to spawn
    ...
    [Header("Prefab References")] public GameObject projectilePrefab;
    public GameObject shadowClonePrefab; // Prefab for shadow clone
    ...
    private void Update()
    {
        if (health <= maxHealth / 1.5 && !isPhaseTwo)
        {
            isPhaseTwo = true; // Switch to phase 2 once health is below 50%
        }
        ...
        switch (currentState)
        {
            ...
            case State.Teleport:
                if (!teleporting)
                {
                    StartCoroutine(TeleportAndClone());
                }
                break;
    ...
    // ================================
    // Teleport and Shadow Clone Phase
    // ================================

    private IEnumerator TeleportAndClone()
    {
        teleporting = true;
        ...
        // Teleport the boss
        transform.position = RandomPointOnScreen(new Vector2(-8, 4), new Vector2(8, -4));

        // Spawn shadow clones in phase 2
        if (isPhaseTwo)
        {
            SpawnShadowClones();
        }
    ...
    private void SpawnShadowClones()
    {
        ...
        {
            Vector3 clonePosition = RandomPointOnScreen(new Vector2(-8, 4), new Vector2(8, -4));
            GameObject clone = Instantiate(shadowClonePrefab, clonePosition, Quaternion.identity);
